...
public class NovelsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.novels_fragment,container,false);
    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        ListView novelsListView=view.findViewById(R.id.NovelsListView);
        Call<List<Book>>call= AppRetrofit.getInstance("booksbytype").getBookViewerApi().getBooksByType("novels");
        call.enqueue(new Callback<List<Book>>() {
            @Override
            public void onResponse(Call<List<Book>> call, Response<List<Book>> response) {
                if(response.body().get(0).isResult()){
                    novelsListView.setAdapter(new BookCardAdapter(view.getContext(),response.body()));
                }
            }
