...
public class TechnologyFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ListView tecnologyListView=view.findViewById(R.id.TecnologyListView);
        Call<List<Book>> call= AppRetrofit.getInstance("booksbytype").getBookViewerApi().getBooksByType("tecnology");
        call.enqueue(new Callback<List<Book>>() {
            @Override
            public void onResponse(Call<List<Book>> call, Response<List<Book>> response) {
                if(response.body().get(0).isResult()){
                    tecnologyListView.setAdapter(new BookCardAdapter(view.getContext(),response.body()));
                }
            }
